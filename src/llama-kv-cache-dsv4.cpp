...
llama_kv_cache_dsv4::llama_kv_cache_dsv4(
    ...
    // Upstream DSV4 MTP+DSpark: the draft-side KV caches carry no next-token
    // prediction layers; keep the fork's per-cache K quant override above.
    ...
    kv_raw = std::make_unique<llama_kv_cache_iswa>(
            model, hparams_raw, type_k_use, type_v,
