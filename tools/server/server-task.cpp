...
common_chat_msg task_result_state::update_chat_msg(
    ...
    try {
        ...
        SRV_WRN("chat parse failed: %s\n", e.what());
        // revert generated_text so the next chunk gets a clean start
        generated_text = generated_text.substr(0, generated_text.size() - text_added.size());
        return chat_msg;
    }
    if (!new_msg.empty()) {
